...
def transform_test_data_scalecano(rgb, intrinsic, data_basic, dtype=torch.float32, device='cuda'):
    """
    ...
    mean = torch.tensor([123.675, 116.28, 103.53]).to(dtype)[:, None, None].to(device)
    std = torch.tensor([58.395, 57.12, 57.375]).to(dtype)[:, None, None].to(device)
    ...
    rgb = torch.from_numpy(rgb.transpose((2, 0, 1))).to(dtype).to(device)
...
def depth_custom_func(
    ...
    indir: str,
    outdir: str,
    config_path: str = './recon/third_party/metric3d/mono/configs/HourglassDecoder/vit.raft5.giant2.py',
    ckpt_path: str = './recon/third_party/metric3d/weight/metric_depth_vit_giant2_800k.pth',
):
    ...
    for an in tqdm(test_data):
        base_fn = '_'.join(an['rgb'].split('/')[-1].split('.')[:-1])
        try:
            rgb_origin = cv2.imread(an['rgb'])[:, :, ::-1].copy()
        except Exception as e:
            print(str(datetime.now()) + ': \033[93mW', f"Failed to read image {an['rgb']} due to {e}", '\033[0m')
            continue
        intrinsic = an['intrinsic']
        with torch.no_grad():
            rgb_input, cam_models_stacks, pad, label_scale_factor = transform_test_data_scalecano(rgb_origin, intrinsic, cfg.data_basic, device=device)
            pred_depth, _, _ = get_prediction(
                model = model,
                input = rgb_input,
        ...
        np.save(os.path.join(outdir, base_fn + '.npy'), pred_depth.detach().cpu().numpy())
